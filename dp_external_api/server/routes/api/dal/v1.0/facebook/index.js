...
router.get('/createCustomAudience', (req, res) => {
    ...
    if (!errorMessage) {
        ...
        else if (!(business_id.toUpperCase() in FACEBOOK_BUSINESS_ACCESS_TOKEN))
    ...
    FacebookAudienceList.findByListId(list_id, STATUS_JOB_LIST.INIT, (audienceList) => {
        ...
        Utils.countLinesInFile(Utils.buildUploadFilePath(list_id), (totalLines) => {
            ...
            audienceList.save().then((audienceListUpdated) => {
                AsyncJobs.unmaskingData(audienceListUpdated)
                    .then((identities) => {
                        AsyncJobs.pushUserToFacebook(audienceListUpdated, identities).then((result) => {
                            let dataToDAL = {};
    ...
    FacebookAudienceList.findByListId(list_id, STATUS_JOB_LIST.INIT, (audienceList) => {
        ...
        Utils.countLinesInFile(Utils.buildUploadFilePath(list_id), (totalLines) => {
            ...
            audienceList.save().then((audienceListUpdated) => {
                AsyncJobs.unmaskingData(audienceListUpdated)
                    .then((identities) => {
                        AsyncJobs.updateUserToFacebook(audienceListUpdated, identities).then((result) => {
                            let dataToDAL = {};
                            if (result.status === 'success') {
                                dataToDAL = {
                                    facebook_audience_id: audienceListUpdated.facebookAudienceListId,
...
router.get('/unmaskingCustomAudience', (req, res) => {
    ...
    AsyncJobs.unMaskFileToFile(filePath, maskedType).then(() => {
    });
    res.send("Processing")
})
...
router.get('/unmaskingFile', (req, res) => {
    const filePath = req.query.filePath;
    const maskedType = req.query.maskedType;
    AsyncJobs.unMaskFile(filePath, maskedType).then(() => {
    });
    res.send("Processing")
})
...
router.put('/createAnalyticAppEvent', (req, res) => {
    ...
    FacebookAnalyticAppEvent.findByListId(list_id, STATUS_JOB_LIST.INIT, (audienceList) => {
        ...
        Utils.countLinesInFile(Utils.buildUploadFilePath(list_id), (totalLines) => {
            ...
            audienceList.status = STATUS_JOB_LIST.PROCESSING;
            audienceList.save().then((audienceListUpdated) => {
                AsyncJobs.readFileContent(audienceListUpdated)
                    .then((events) => {
                        AsyncJobs.pushAppEventToFacebook(audienceListUpdated, events).then((result) => {
                            if (result.status === 'success') {
