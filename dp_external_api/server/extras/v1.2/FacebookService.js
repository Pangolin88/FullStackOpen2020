...
const pool = new Pool(MASKING_DB_MAPPING_CONNECTION);
export default class FacebookService {
	static unmaskingData(audienceList) {
		...
		const nullLookupFile = fs.createWriteStream(nullLookUpFilePath, {
			flags: 'w',
		});
			...
			lineReader.on('line', function (line) {
				...
				redisClient.hget('mapping:uid_' + maskedType, line, (error, unmaskedData) => {
					...
					if (!unmaskedData || unmaskedData == 'None') {
						pool.query(
							`SELECT normalize_${maskedType} FROM public.uid_${maskedType} WHERE uid_${maskedType} = ${line}`,
							(err, res) => {
								if (!err && res.rows[0]) {
									redisClient.hset(
										'mapping:uid_' + maskedType,
										line,
										res.rows[0].normalize_device_id
									);
									let result = res.rows[0].normalize_device_id.toLowerCase();
									if (maskedType === 'phone') result = result.replace('+84', '84');
									...
									if (lineCount === audienceList.countReceived) return processResult();
								} else {
									nullLookupDBCount += 1;
						...
						nullLookupDBCount += 1;
						nullLookupFile.write(line + '\n');
						if (lineCount === audienceList.countValid) return processResult();
					} else {
	...
	static unMaskFileToFile(maskedFilePath, maskedType) {
		...
		const maskedFile = fs.createWriteStream(
			Utils.buildUnmaskedDataFile('result_' + new Date().toDateString()),
			{ flags: 'w' }
		);
		let lineCount = 0;
		let nullLookupDBCount = 0;
					...
					redisClient.hget('mapping:uid_' + maskedType, line, (error, unmaskedData) => {
						...
						if (!unmaskedData || unmaskedData == 'None') {
							pool.query(
								...
								(err, res) => {
									if (!err) {
										console.log(res.rows[0], lineCount);
										if (res.rows[0]) {
											redisClient.hset(
												'mapping:uid_' + maskedType,
												line,
							...
							nullLookupDBCount += 1;
							nullLookupFile.write(line + '\n');
							if (lineCount === totalLines) return processResult();
						} else {
							let result = unmaskedData.toLowerCase();
							if (maskedType === 'phone') result = result.replace('+84', '84');
					...
					try {
						pool.query(
							...
							(err, res) => {
								if (!err) {
									if (res.rows[0]) {
										if (lineCount % 10000 ===0) console.log(res.rows[0].normalize_device_id)
										redisClient.hset(
											'mapping:uid_' + maskedType,
											line,
											res.rows[0].normalize_device_id
	...
	static readFileContent(audienceList) {
		...
		return new Promise(function (resolve, reject) {
			...
			lineReader.on('line', function (line) {
				...
				lineCount += 1;
				if (previews.length < UNMASKED_DATA_PREVIEW_COUNT) previews.push(line);
				try {
	...
	static pushUserToFacebook(audienceList, identities) {
		...
		const audienceName =
			'Shared by DP - ' + facebookAudienceListName + ' ' + Utils.generateShortUUID();
		...
		const pushUser = (resolve, facebookAudienceId) => {
			const submitToFacebook = function (users) {
				...
				const callbackSuccess = (result) => {
					...
					if (result.num_invalid_entries > 0)
						message =
							message +
							' Invalid:' +
							result.num_invalid_entries +
							' Invalid sample:' +
							JSON.stringify(result.invalid_entry_samples);
				...
				setTimeout(() => {
					...
					Utils.makeHttpRequest(
						'post',
						FACEBOOK_API.UPDATE_CUSTOM_AUDIENCE.replace(':CUSTOM_AUDIENCE_ID', facebookAudienceId),
						callbackSuccess,
						callbackError,
						data,
						headers,
			...
			for (let line of identities) {
				...
				if (
					submitList.length == FACEBOOK_UPDATE_CUSTOM_AUDIENCE_PER_TIME ||
					lineCount == countValid
				) {
		...
		return new Promise(function (resolve, reject) {
			const callbackSuccess = (facebookResult) => {
				if (!Utils.safeGet(facebookResult, 'id')) {
					Utils.logErrorToFileForAudience(
						'Cannot create Audience List on Facebook: ' + facebookResult
					);
					resolve({
						status: 'failed',
			...
			Utils.makeHttpRequest(
				'post',
				FACEBOOK_API.CREATE_CUSTOM_AUDIENCE.replace(':AD_ACCOUNT_ID', ad_account_id),
				callbackSuccess,
				callbackError,
				dataFacebook
			);
				...
				return new Promise((resolve, reject) => {
					...
					lineReader.on('line', function (line) {
						...
						if (lineCount % 100000 === 0)
							Utils.logToFileForAudience(`Hashing ${lineCount} elements`);
						++lineCount;
	...
	static updateUserToFacebook(audienceList, identities) {
		...
		Utils.logToFileForAudience(
			'Start to update for Facebook Audience ID: ' + facebookAudienceListId
		);
		const access_token = FACEBOOK_BUSINESS_ACCESS_TOKEN[businessId.toUpperCase()];
		const pushUser = (resolve, facebookAudienceId) => {
			const submitToFacebook = function (users) {
	...
	static updateUserRawToFacebook(audience) {
		...
		hashingIdentities(filePath).then((identities) => {
			...
			Utils.logToFileForAudience(`List of users: ${identities}`);
		});
	}
		...
		const pushEvent = (resolve) => {
			const submitToFacebook = function (event) {
				...
				setTimeout(() => {
					Utils.makeHttpRequest(
						'post',
						FACEBOOK_API.PUSH_APP_EVENT.replace(':APP_ID', facebookAppId),
						callbackSuccess,
						callbackError,
						data,
						headers,
			...
			Utils.logToFileForAnalyticAppEvent(
				'----- Start: pushing ' + countReceived + ' events to Facebook'
			);
			var lineCount = 0;
			for (let event of events) {
				...
				if (lineCount === countReceived) {
					Utils.logToFileForAnalyticAppEvent(
						'----- Finish: pushing ' + countReceived + ' events to Facebook'
					);
					resolve({
						status: 'success',
					});
	...
	static clearUserCustomAudience(audienceId) {
		...
		return Utils.makeHttpRequest(
			'delete',
			FACEBOOK_API.UPDATE_CUSTOM_AUDIENCE.replace(':CUSTOM_AUDIENCE_ID', audienceId),
			callbackSuccess,
			callbackError,
			data,
			headers,
	...
	static pushRawAppEvent(eventName, appId, advertisingId, extInfo, currency, event) {
		return new Promise((resolve) => {
			...
			Utils.makeHttpRequest(
				'post',
				FACEBOOK_API.PUSH_APP_EVENT.replace(':APP_ID', appId),
				callbackSuccess,
				callbackError,
				data,
				headers,
	...
	static async unmaskDeviceId(deviceIds) {
		const redisClient = redis.createClient(REDIS_CONNECTION);
		return await Promise.all(
			deviceIds.map(async device => {
				let data = await new Promise((resolve, reject) => {
					redisClient.hget('mapping:uid_device_id', device, (err, value) => {
						if (!err) resolve(value)
					});
				})
				if (!data) {
					const result = await pool.query(`SELECT normalize_device_id FROM public.uid_device_id WHERE uid_device_id = ${device}`);
					data = result.rows[0] ? result.rows[0] : device;
				}
